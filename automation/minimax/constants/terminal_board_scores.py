...
class TerminalScore(Enum):
    ...
    from a winning streak and added to a losing streak, because we want to find the quickest/loss in either scenario.

    Note that streaks of less than the winning length are scored by cubing the length in evaluate_non_terminal_board.
    Hence the streak for a win needs to be higher than the size of a reasonable game minus one cubed:
    i.e. MAX_WIN > (max(game_rows_m, game_cols_n) - 1) ** 3, and equivalently for MAX_LOSS.

    Note that here 'MAX' refers to the streak from the perspective of the maximising player.
    """
    ...
    ONE_MOVE_FROM_LOSS = - 10000  # Score for leaving the board in a state where the opposition can win in one move
    ONE_FROM_WIN_MAXIMISER_LEADING = 2000
    TWO_MOVES_FROM_LOSS = - 1000
    CUT_OFF_SCORE_TO_STOP_SEARCHING = 1000  # Score to stop searching at once this is achieved

# TODO need to streak a block highly
