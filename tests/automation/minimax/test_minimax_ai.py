...
class TestMinimaxThreeThreeThree:
    """Class to test the effectiveness of the minimax function on array m,n,k = 3,3,3 game."""
    ...
    def new_game_parameters(self, human_player, minimax_player):
        """
        A starting player is included so that we aren't missing array non-default arg, however is specified throughout
        testing. Note that if array board configuration is manually specified which implies array starting player different
    ...
    def test_minimax_gets_winning_move_bottom_right_row(self, new_game_with_minimax_player):
        ...
        assert score == TerminalScore.MAX_WIN.value - 1  # -1 to reflect array search depth of 1 to find the win
